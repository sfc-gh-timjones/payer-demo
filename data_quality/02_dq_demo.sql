...
SELECT
  ...
  ARGUMENT_NAMES                             AS column_checked,
  EXPECTATION_EXPRESSION                     AS pass_threshold,
  VALUE                                      AS current_value,
  MEASUREMENT_TIME                           AS detected_at
FROM SNOWFLAKE.LOCAL.DATA_QUALITY_MONITORING_EXPECTATION_STATUS
WHERE TABLE_NAME         = 'MEMBER'
  AND TABLE_SCHEMA       = 'SILVER'
  AND TABLE_DATABASE     = 'ZZFACETS_DEV_CLONE'
...
FROM latest
ORDER BY
  CASE WHEN METRIC_NAME IN ('DUPLICATE_COUNT', 'MEDICAID_MISSING_BIC_COUNT') THEN 1
       WHEN METRIC_NAME IN ('NULL_COUNT', 'INVALID_NPI_COUNT') THEN 2
       ELSE 3 END,
  VALUE DESC;
