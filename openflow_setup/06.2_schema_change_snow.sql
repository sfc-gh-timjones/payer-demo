-- =============================================================================
-- FILE: 06.2_schema_change_snow.sql
-- PURPOSE: Run before and after the schema change in 06_schema_change_mssql.sql
--          to show Snowflake automatically picked up the new NWNW_REGION column
--          via Openflow schema evolution.
--
-- RUN ONCE before making the SQL Server change, then run again after Openflow
-- ingests the new records to see the column appear automatically.
-- =============================================================================

-- Show all columns on the Bronze table (NWNW_REGION absent before, present after)
DESCRIBE TABLE FACETS_BRONZE.RAW.CMC_NWNW_NETWORK;

-- Show all rows (original ~20 rows pre-change; ~25 rows with NWNW_REGION post-change)
SELECT * FROM FACETS_BRONZE.RAW.CMC_NWNW_NETWORK ORDER BY NWNW_ID;
